  ...
  <div class="container my-[40px]">
    ...
    <UIBox v-else class="grid md:grid-cols-2 grid-cols-1 gap-4">
      ...
      <div v-if="orderDetails?.hasOwnProperty('shell_type')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Shell Type: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.shell_type }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('shell_url')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Shell URL: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.shell_url }} </p>
      </div>
      <div v-if="orderDetails?.status" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Status: </p>
        <span class="text-[18px] font-medium text-white leading-5 px-3 pt-2 pb-2 rounded-[4px]"
      ...
      <div v-if="orderDetails?.hasOwnProperty('is_deleted')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Is Deleted: </p>
      ...
      <div v-if="orderDetails?.hasOwnProperty('category')" class="flex items-center gap-2">
        ...
        <p class="font-medium text-[18px]">{{ orderDetails.category }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('country')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Country: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.country }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('location')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Location: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.location }} </p>
      </div>
      <!--      <div v-if="orderDetails?.hasOwnProperty('details')" class="flex items-center gap-2">-->
      <!--        <p class="font-bold text-[18px]">Details: </p>-->
      <!--        <p class="font-medium text-[18px]">{{ orderDetails.details }} </p>-->
      <!--      </div>-->
      <div v-if="orderDetails?.hasOwnProperty('domain')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Domain: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.domain }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('proof')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Proof: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.proof }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('type')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Type: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.type }} </p>
      </div>
      <div v-if="orderDetails?.hasOwnProperty('notes')" class="flex items-center gap-2">
        <p class="font-bold text-[18px]">Notes: </p>
        <p class="font-medium text-[18px]">{{ orderDetails.notes }} </p>
      </div>
